...
import { useAdminCoursesCreateModalT } from '@/i18n';
...
interface CreateCourseModalProps {
  onCreate: (
    course: Omit<AdminCourseRow, 'id' | 'enrolled' | 'rating' | 'createdAt'>,
  ) => void;
  onClose: () => void;
}
...
export function CreateCourseModal({
  ...
  const t = useAdminCoursesCreateModalT();
  ...
  return (
    ...
    >
      <form
        ...
        onSubmit={(e) => {
          e.preventDefault();
          onCreate({ title, instructor: educator, category, level, status });
        }}
        className="space-y-4"
      >
        ...
        <div className="flex flex-col gap-1.5">
          <label className={labelCls}>{t('educator')}</label>
          <input
            value={educator}
            onChange={(e) => setEducator(e.target.value)}
            placeholder={t('educatorPlaceholder')}
            className={inputCls}
          />
        </div>
