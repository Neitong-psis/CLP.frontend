  ...
  return (
    <div className="border-border bg-card rounded-2xl border p-5 shadow-sm sm:p-6">
      ...
      <div ref={ref} className="flex gap-2 sm:gap-3">
        {/* Y-axis labels — outside the scroll region, so they stay visible
            while the bars scroll horizontally on narrow screens. */}
        <div className="relative flex h-56 w-8 shrink-0 flex-col justify-between pb-6 sm:h-72 sm:w-10">
          {Y_TICKS.map((v) => (
            <span
              key={v}
        ...
        {/* Chart area — bars get a minimum width and scroll horizontally
            below sm instead of being squeezed to unreadable slivers; at
            sm+ there's room for all 12 to breathe natively. */}
        <div className="scrollbar-none relative flex-1 overflow-x-auto">
          <div className="relative min-w-150 sm:min-w-0">
            {/* Grid lines */}
