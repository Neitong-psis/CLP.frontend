export const ADMIN_USER = {
  role: 'Admin',
};

export type UserStatus = 'Active' | 'Inactive' | 'Suspended' | 'Achieved';
export type UserRole = 'Learner' | 'Educator' | 'Admin';
export type InviteStatus = 'Approved' | 'Pending';

export interface AdminUserRow {
  id: string;
  name: string;
  email: string;
  role: UserRole;
  status: UserStatus;
  inviteStatus: InviteStatus;
  enrolled: number;
  joined: string;
  lastActive: string;
  username?: string;
  phone?: string;
  studentId?: string;
  program?: string;
  organization?: string;
  gender?: string;
  dob?: string;
  nationality?: string;
  address?: string;
  bio?: string;
}

export const ADMIN_USERS: AdminUserRow[] = [
  {
    id: 'u1',
    name: 'John Doe',
    email: 'john@example.com',
    role: 'Learner',
    status: 'Active',
    inviteStatus: 'Approved',
    enrolled: 5,
    joined: 'Jan 15, 2024',
    lastActive: '2h ago',
  },
  {
    id: 'u2',
    name: 'Dr. Sopheak Chan',
    email: 'sopheak@example.com',
    role: 'Educator',
    status: 'Active',
    inviteStatus: 'Approved',
    enrolled: 0,
    joined: 'Jun 20, 2023',
    lastActive: '3h ago',
  },
  {
    id: 'u3',
    name: 'Sarah Wilson',
    email: 'sarah@example.com',
    role: 'Admin',
    status: 'Active',
    inviteStatus: 'Approved',
    enrolled: 0,
    joined: 'Mar 10, 2023',
    lastActive: '1d ago',
  },
  {
    id: 'u4',
    name: 'Mike Johnson',
    email: 'mike@example.com',
    role: 'Learner',
    status: 'Achieved',
    inviteStatus: 'Approved',
    enrolled: 12,
    joined: 'Feb 28, 2024',
    lastActive: '5d ago',
  },
  {
    id: 'u5',
    name: 'Dr. Ratanak Sok',
    email: 'ratanak@example.com',
    role: 'Educator',
    status: 'Active',
    inviteStatus: 'Approved',
    enrolled: 0,
    joined: 'Aug 12, 2023',
    lastActive: '5d ago',
  },
  {
    id: 'u6',
    name: 'Priya Sharma',
    email: 'priya@example.com',
    role: 'Learner',
    status: 'Inactive',
    inviteStatus: 'Pending',
    enrolled: 3,
    joined: 'Mar 5, 2025',
    lastActive: '2w ago',
  },
  {
    id: 'u7',
    name: 'Tom Chen',
    email: 'tom@example.com',
    role: 'Learner',
    status: 'Active',
    inviteStatus: 'Pending',
    enrolled: 8,
    joined: 'Apr 1, 2025',
    lastActive: '30m ago',
  },
];

export type CourseStatus = 'Public' | 'Pending' | 'Archive';

export interface AdminCourseRow {
  id: string;
  title: string;
  category: string;
  level: string;
  instructor: string;
  enrolled: number;
  rating: number;
  status: CourseStatus;
  createdAt: string;
  dueDate?: string;
  priority?: string;
  workProgress?: number;
  workStatus?: string;
  /** Present when sourced from the live API (`toAdminCourseRow`) — powers
   *  the course-builder wizard's edit mode, which the mock fixtures below
   *  never feed into. */
  description?: string;
  price?: number;
  thumbnail?: string;
  categoryId?: string;
  instructorId?: string;
}

export const ADMIN_COURSES: AdminCourseRow[] = [
  {
    id: 'ac1',
    title: 'Complete Khmer Literature Bootcamp',
    category: 'Khmer Literature',
    level: 'Beginner',
    instructor: 'Dr. Sopheak Chan',
    enrolled: 234567,
    rating: 4.8,
    status: 'Archive',
    createdAt: 'May 13, 2026',
    dueDate: '2026-06-30',
    priority: 'medium',
    workProgress: 0,
    workStatus: 'archived',
  },
  {
    id: 'ac2',
    title: 'Child Psychology A-Z: Growth & Behavior',
    category: 'Child Development',
    level: 'Intermediate',
    instructor: 'Dr. Ratanak Sok',
    enrolled: 189432,
    rating: 4.9,
    status: 'Public',
    createdAt: 'May 12, 2026',
    dueDate: '2026-05-12',
    priority: 'high',
    workProgress: 100,
    workStatus: 'published',
  },
  {
    id: 'ac3',
    title: 'Advanced Leadership Certification',
    category: 'Leadership Development',
    level: 'Advanced',
    instructor: 'Vandy Chhorn',
    enrolled: 156789,
    rating: 4.7,
    status: 'Public',
    createdAt: 'May 11, 2026',
    dueDate: '2026-05-10',
    priority: 'high',
    workProgress: 100,
    workStatus: 'published',
  },
  {
    id: 'ac4',
    title: 'The Complete Khmer Literature Course',
    category: 'Khmer Literature',
    level: 'Beginner',
    instructor: 'Sok Chanthou',
    enrolled: 345678,
    rating: 4.9,
    status: 'Public',
    createdAt: 'May 10, 2026',
    dueDate: '2026-05-20',
    priority: 'medium',
    workProgress: 100,
    workStatus: 'published',
  },
  {
    id: 'ac5',
    title: 'Full Community Leadership Program',
    category: 'Leadership',
    level: 'Intermediate',
    instructor: 'Dara Kim',
    enrolled: 298765,
    rating: 4.8,
    status: 'Pending',
    createdAt: 'May 9, 2026',
    dueDate: '2026-07-25',
    priority: 'high',
    workProgress: 45,
    workStatus: 'writing',
  },
  {
    id: 'ac6',
    title: 'Foundations of Child Development',
    category: 'Child Development',
    level: 'Beginner',
    instructor: 'Dr. Ratanak Sok',
    enrolled: 187654,
    rating: 4.7,
    status: 'Public',
    createdAt: 'May 8, 2026',
    workStatus: 'published',
  },
  {
    id: 'ac7',
    title: 'Leadership Development for Educators',
    category: 'Leadership Development',
    level: 'Intermediate',
    instructor: 'Vandy Chhorn',
    enrolled: 134567,
    rating: 4.8,
    status: 'Public',
    createdAt: 'May 7, 2026',
    workStatus: 'published',
  },
  {
    id: 'ac8',
    title: 'Innovative Learning Essentials',
    category: 'Innovative Learning',
    level: 'Beginner',
    instructor: 'Sophea Ly',
    enrolled: 98765,
    rating: 4.6,
    status: 'Archive',
    createdAt: 'May 6, 2026',
    workStatus: 'archived',
  },
  {
    id: 'ac9',
    title: 'Advanced Innovative Learning Patterns',
    category: 'Innovative Learning',
    level: 'Advanced',
    instructor: 'Sophea Ly',
    enrolled: 85432,
    rating: 4.9,
    status: 'Pending',
    createdAt: 'May 5, 2026',
    workStatus: 'todo',
  },
  {
    id: 'ac10',
    title: 'Community Leadership Playbook',
    category: 'Leadership',
    level: 'Intermediate',
    instructor: 'Dara Kim',
    enrolled: 125432,
    rating: 4.9,
    status: 'Public',
    createdAt: 'May 4, 2026',
    workStatus: 'published',
  },
  {
    id: 'ac11',
    title: 'Complete Storytelling Bootcamp',
    category: 'Khmer Literature',
    level: 'Beginner',
    instructor: 'Chenda Prak',
    enrolled: 456789,
    rating: 4.8,
    status: 'Public',
    createdAt: 'May 3, 2026',
    workStatus: 'published',
  },
];

export interface ApprovalQueueItem {
  id: string;
  title: string;
  instructor: string;
  category: string;
  lessons: number;
  submittedAt: string;
}

export const APPROVAL_QUEUE: ApprovalQueueItem[] = [
  {
    id: 'aq1',
    title: 'Advanced Khmer Poetry: Form, Meter, Performance',
    instructor: 'Chenda Prak',
    category: 'Khmer Literature',
    lessons: 42,
    submittedAt: 'Jun 27, 2026',
  },
  {
    id: 'aq2',
    title: 'Innovative Learning Design Lab',
    instructor: 'Sophea Ly',
    category: 'Innovative Learning',
    lessons: 58,
    submittedAt: 'Jun 26, 2026',
  },
  {
    id: 'aq3',
    title: 'Deep Child Psychology: Attachment & Behavior',
    instructor: 'Dr. Ratanak Sok',
    category: 'Child Development',
    lessons: 35,
    submittedAt: 'Jun 25, 2026',
  },
];

export const DASHBOARD_STATS = [
  {
    label: 'Active Courses',
    value: '7',
    change: '+8%',
    icon: 'book',
    iconColor: 'bg-emerald-600 text-white',
    desc: '11 total courses',
  },
  {
    label: 'Total Users',
    value: '7',
    change: '+12%',
    icon: 'users',
    iconColor: 'bg-blue-600 text-white',
    desc: 'From live user records',
  },
  {
    label: 'Monthly Revenue',
    value: '$98.6k',
    change: '+18%',
    icon: 'dollar',
    iconColor: 'bg-orange-500 text-white',
    desc: 'May 2026',
  },
  {
    label: 'Total Enrollments',
    value: '2,313,870',
    change: '+28%',
    icon: 'trending',
    iconColor: 'bg-violet-600 text-white',
    desc: 'All-time enrollments',
  },
  {
    label: 'Total Issued Certificates',
    value: '12,450',
    change: '+9%',
    icon: 'award',
    iconColor: 'bg-amber-500 text-white',
    desc: 'Total issued to date',
  },
] as const;

export const USER_DISTRIBUTION = [
  { label: 'Admins', count: 1, color: '#f4a300' },
  { label: 'Educators', count: 2, color: '#10b981' },
  { label: 'Learners', count: 4, color: '#3b82f6' },
];

export type TopCourseVisibility = 'Public' | 'Pending' | 'Archive';

export const TOP_PERFORMING_COURSES = [
  {
    id: 'ac11',
    title: 'Complete Storytelling Bootcamp',
    instructor: 'Chenda Prak',
    category: 'Khmer Literature',
    level: 'Beginner',
    students: '315,758',
    visibility: 'Public' as TopCourseVisibility,
    progress: 90,
    rating: 4.8,
    revenue: '$84.2k',
  },
  {
    id: 'ac4',
    title: 'The Complete Khmer Literature Course',
    instructor: 'Sok Chanthou',
    category: 'Khmer Literature',
    level: 'Beginner',
    students: '245,678',
    visibility: 'Public' as TopCourseVisibility,
    progress: 78,
    rating: 4.9,
    revenue: '$65.5k',
  },
  {
    id: 'ac5',
    title: 'Full Community Leadership Program',
    instructor: 'Dara Kim',
    category: 'Leadership',
    level: 'Intermediate',
    students: '178,765',
    visibility: 'Pending' as TopCourseVisibility,
    progress: 65,
    rating: 4.8,
    revenue: '$48.1k',
  },
  {
    id: 'ac7',
    title: 'Leadership Development Certification',
    instructor: 'Vandy Chhorn',
    category: 'Leadership Development',
    level: 'Intermediate',
    students: '234,567',
    visibility: 'Archive' as TopCourseVisibility,
    progress: 72,
    rating: 4.7,
    revenue: '$62.3k',
  },
  {
    id: 'ac2',
    title: 'Child Psychology A-Z: Growth & Behavior',
    instructor: 'Dr. Ratanak Sok',
    category: 'Child Development',
    level: 'Intermediate',
    students: '190,810',
    visibility: 'Public' as TopCourseVisibility,
    progress: 60,
    rating: 4.9,
    revenue: '$51.2k',
  },
];

export const REVENUE_STATS = [
  { label: 'Annual Revenue', value: '$806.5k', change: '+53%' },
  { label: 'This Month', value: '$98.6k', change: '+15%' },
  { label: 'Monthly Recurring Revenue', value: '$98.6k', change: '+46%' },
  { label: 'Avg Order Value', value: '$43', change: '+14%' },
] as const;

export const MONTHLY_REVENUE = [
  { month: 'Jun', amount: 38_000 },
  { month: 'Jul', amount: 42_000 },
  { month: 'Aug', amount: 45_000 },
  { month: 'Sep', amount: 52_000 },
  { month: 'Oct', amount: 58_000 },
  { month: 'Nov', amount: 63_000 },
  { month: 'Dec', amount: 71_000 },
  { month: 'Jan', amount: 75_000 },
  { month: 'Feb', amount: 82_000 },
  { month: 'Mar', amount: 88_000 },
  { month: 'Apr', amount: 91_000 },
  { month: 'May', amount: 98_600 },
];

export const REVENUE_BY_CATEGORY = [
  { name: 'Khmer Literature', amount: '$60.7k', pct: 100 },
  { name: 'Leadership Development', amount: '$49.1k', pct: 81 },
  { name: 'Child Development', amount: '$41.2k', pct: 68 },
  { name: 'Leadership', amount: '$33.9k', pct: 56 },
  { name: 'Innovative Learning', amount: '$21.9k', pct: 36 },
];

export const QUIZ_ANALYTICS = [
  {
    title: 'Khmer Grammar Fundamentals Quiz',
    difficult: 'Classifier particles',
    completion: 88,
    avgScore: 79,
    passRate: 74,
  },
  {
    title: 'Child Safety Assessment',
    difficult: 'Recognizing developmental red flags',
    completion: 70,
    avgScore: 65,
    passRate: 61,
  },
  {
    title: 'Leadership Styles Checkpoint',
    difficult: 'Situational leadership tradeoffs',
    completion: 93,
    avgScore: 85,
    passRate: 90,
  },
] as const;

export const PLATFORM_ANALYTICS_DATA = [
  { month: 'Jun', users: 12000, enrollments: 28000 },
  { month: 'Jul', users: 18000, enrollments: 34000 },
  { month: 'Aug', users: 22000, enrollments: 40000 },
  { month: 'Sep', users: 28000, enrollments: 46000 },
  { month: 'Oct', users: 32000, enrollments: 50000 },
  { month: 'Nov', users: 36000, enrollments: 55000 },
  { month: 'Dec', users: 40000, enrollments: 60000 },
  { month: 'Jan', users: 44000, enrollments: 65000 },
  { month: 'Feb', users: 48000, enrollments: 70000 },
  { month: 'Mar', users: 52000, enrollments: 75000 },
  { month: 'Apr', users: 56000, enrollments: 80000 },
  { month: 'May', users: 60000, enrollments: 85000 },
];

export const RECENT_ENROLLMENTS = [
  {
    user: 'Alex Johnson',
    course: 'Complete Khmer Literature Bootcamp',
    date: 'May 20, 2026',
    status: 'Active',
  },
  {
    user: 'Sopheaktra Meng',
    course: 'Child Psychology A-Z',
    date: 'May 19, 2026',
    status: 'Active',
  },
  {
    user: 'John Doe',
    course: 'Advanced Leadership Certification',
    date: 'May 18, 2026',
    status: 'Active',
  },
  {
    user: 'Jane Smith',
    course: 'Full Community Leadership Program',
    date: 'May 17, 2026',
    status: 'Inactive',
  },
  {
    user: 'Sarah Lee',
    course: 'The Complete Khmer Literature Course',
    date: 'May 16, 2026',
    status: 'Active',
  },
];

export const WEEKLY_ENROLLMENTS = [
  { day: 'Mon', count: 42 },
  { day: 'Tue', count: 68 },
  { day: 'Wed', count: 55 },
  { day: 'Thu', count: 90 },
  { day: 'Fri', count: 73 },
  { day: 'Sat', count: 28 },
  { day: 'Sun', count: 15 },
];

export const TOP_COURSES = [
  { title: 'Complete Storytelling Bootcamp', enrolled: 315758, completion: 75 },
  {
    title: 'Complete Khmer Literature Course',
    enrolled: 245678,
    completion: 72,
  },
  { title: 'Khmer Literature Bootcamp', enrolled: 234567, completion: 68 },
  {
    title: 'Full Community Leadership Program',
    enrolled: 178765,
    completion: 58,
  },
  { title: 'Child Psychology A-Z', enrolled: 190810, completion: 45 },
];

export * from './certifications';
export * from './courses';
export * from './users';
